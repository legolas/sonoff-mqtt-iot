...
public class ScheduleCoordinator {
    ...
    public void startUp() {
        ...
        try {
            ...
            scheduler.scheduleJob(
                createJob(String.format("%s0", SWITCH_OFF), SONOFF1, SwitchJob.OFF),
                createTrigger(String.format("%s0", SWITCH_OFF), SONOFF1, "0 0 23 ? * MON,TUE,WED,THU,SUN *")
            );

            scheduler.scheduleJob(
                createJob(String.format("%s1", SWITCH_OFF), SONOFF1, SwitchJob.OFF),
                createTrigger(String.format("%s1", SWITCH_OFF), SONOFF1, "0 30 23 ? * FRI,SAT *")
            );
